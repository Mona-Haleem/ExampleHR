...
export class TimeOffService {
  ...
  async getBalance(employeeId: string, locationId: string) {
    ...
    const reservedBalance = await this.getActiveRequestsDates(employeeId, locationId) || 0;
  ...
  private async getActiveRequestsDates(employee_id: string, location_id: string) {
    ...
    return request?.flatMap(request => request.requestedDates)?.length || 0;
  }
  ...
  async syncSingleRequest(request: TimeOffRequest): Promise<string> {
    ...
    if (balanceInfo.availableBalance < 0) {
      await this.updateStatus(request.request_id, RequestStatus.REJECTED);
      return 'REJECTED';
    }
